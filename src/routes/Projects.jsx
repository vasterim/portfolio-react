  ...
  return (
    <div className='row flex w-full justify-center flex-col items-center'>
      ...
      <ul className="flex justify-evenly flex-wrap">
              {
                projects.map((project, key) => <div key={key} className='bg-black w-[500px] min-h-[500px] h-max rounded-lg p-10 max-w-lg text-center h-max border border-white border-opacity-0 mx-16 my-16 flex flex-col justify-start items-center'><img src={project.photo} alt="" className='w-[140px] h-auto mb-5'/> <h1 className='font-bold'>{project.projectName} | id: {key}</h1> <p className='my-[20px]'>{project.description}</p></div>)
              }
      </ul>
